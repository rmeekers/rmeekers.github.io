...
function render(resume) {
    ...
    _.each( resume.education, function( education_info ) {
        ...
        var study_in_progress,
            start_date = education_info.startDate && new Date( education_info.startDate ),
            end_date = education_info.endDate && new Date( education_info.endDate );

        if ( start_date ) {
            education_info.startDate = moment( start_date ).format( date_format );
        }

        if ( end_date ) {
            education_info.endDate = moment( end_date ).format( date_format );
        }

        study_in_progress = !! end_date;
        end_date = end_date || new Date();
        education_info.duration = humanizeDuration(
            moment.duration( end_date.getTime() - start_date.getTime() ).add(1, "days"),
            study_in_progress )
    });
